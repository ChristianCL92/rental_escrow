...
pub fn handler(
    ...
    require!(
        apartment_id >= MIN_APARTMENT_NUMBER && apartment_id <= MAX_APARTMENT_NUMBER,
        EscrowError::InvalidApartmentId
    );

    require!(amount >= MIN_PAYMENT_AMOUNT, EscrowError::InvalidAmount);

    let clock = Clock::get()?;
    let seconds_in_day = 86400;
    let start_of_day = ((clock.unix_timestamp as u64) / seconds_in_day) * seconds_in_day;
    require!(rent_time >= start_of_day, EscrowError::InvalidRentTime);
