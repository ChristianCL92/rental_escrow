...
const owner = anchor.web3.Keypair.generate() */
  ...
  it('Should create USDC mint and fund guest', async () => {
    ...
    console.log(
      'Mint address for USDC created successfully',
      usdcMint.toString(),
    );
    ...
    const guestAccountInfo = await getAccount(
      provider.connection,
      guestTokenAccount.address,
    );

    console.log(
      'Guest has the following USDC balance:',
      Number(guestAccountInfo.amount) / 1_000000,
      'USDC',
    );
  });
  ...
  it('Should create the PDA for escrow-account and fund it with UDSC', async () => {
    ...
    const tx = await program.methods
      .initialize(apartmentId, amount, rentTime)
      .accounts({
    ...
    // Check balances
    const guestBalance = await getAccount(
      provider.connection,
      guestTokenAccount.address,
    );
  ...
  it('Should fail to release the payment before the rent time', async () => {
    ...
    console.log(
      '💼 Owner token account created:',
      ownerTokenAccount.address.toString(),
    );
  ...
  it('Should successfully release payment after rent_time', async () => {
    ...
    console.log(
      '  Escrow:',
      Number(escrowBalanceBefore.amount) / 1_000000,
      'USDC',
    );
    console.log(
      '  Owner:',
      Number(ownerBalanceBefore.amount) / 1_000000,
      'USDC',
    );
    ...
    console.log(
      'Owner token balance after:',
      Number(ownerBalanceAfter.amount) / 1_000000,
      'USDC',
    );
    assert.equal(
  ...
  it('Should not release payment TWICE', async () => {
    ...
    try {
      ...
      console.log(
        'Cannot release twice because account is closed',
        err.message,
      );
    }
  });
  ...
  it('should create a second escrow for testing..', async () => {
    ...
    console.log(
      'escrow token account2 created with address:',
      escrowTokenAccount2.address.toString(),
    );
    ...
    const tx = await program.methods
      .initialize(apartmentId2, amount, rentTime2)
      .accounts({
    ...
    const escrowBalance = await getAccount(
      provider.connection,
      escrowTokenAccount2.address,
    );
    console.log(
      'Balance inside token escrow account',
      Number(escrowTokenAccount2.amount) / 1_000000,
      'USDC',
    );

    assert.equal(
      Number(escrowBalance.amount),
      500_000000,
      'Escrow 2 should hold 500 USDC',
    );
  });

  it('Should successfully cancel booking before check-in', async () => {
    const balanceBeforeCancel = await getAccount(
      provider.connection,
      guestTokenAccount.address,
    );
    console.log(
      'Balance before canceling room is',
      Number(balanceBeforeCancel.amount) / 1_000000,
      'USDC',
    );
    ...
    const balanceAfterCancel = await getAccount(
      provider.connection,
      guestTokenAccount.address,
    );
    console.log(
      '  Guest balance AFTER:',
      Number(balanceAfterCancel.amount) / 1_000000,
      'USDC',
    );

    const fundsReceived =
      Number(balanceAfterCancel.amount) - Number(balanceBeforeCancel.amount);
    console.log('Funds received', fundsReceived / 1_000000, 'USDC');

    assert.equal(
      fundsReceived,
      500_000000,
      'Guest should receive 500 USDC refund',
    );
  ...
  it('should fail to cancel booking after check-in started', async () => {
    ...
    await program.methods
      .initialize(apartmentId3, amount, rentTime3)
      .accounts({
