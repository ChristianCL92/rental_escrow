...
const useBookingAPI = () => {
  ...
  const datesBooked = useCallback(async (apartmentId: number) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "failed to get booked dates";
      setError(message);
      throw error;
    }
  }, []);
  ...
  const findBookingId = useCallback(
    async (apartmentId: number, checkInDate: Date) => {
      if (!publicKey) {
      ...
      try {
        ...
        const message =
          error instanceof Error ? error.message : "failed to get bookingId";
        setError(message);
        throw error;
      }
    },
  ...
  const rollbackBooking = useCallback(
    async (bookingId: string) => {
      ...
      try {
        ...
        const message =
          error instanceof Error ? error.message : "Failed to rollback booking";
        setError(message);
        throw error;
      } finally {
        setIsRollback(false);
