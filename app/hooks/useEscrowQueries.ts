...
import useRentalProgram, {EscrowInfo, ReleasePaymentParams} from "./useRentalProgram"
import { PublicKey } from "@solana/web3.js";

const useEscrowQueries = () => {
    const queryClient = useQueryClient();
    const { releasePayment, program, isOwner, fromUSDCAmount } = useRentalProgram();
      ...
      return {    
        ...
        staleTime: 30 * 1000,
         retry: (failureCount, error) => {
           if(error instanceof Error && error.message === "Program not initialized") {
             return false
           }
